...
//the old value is used to achieve more flexibility with the sensor reading
//see "read_sensors" function for more details

int front_sensor;
int front_sensor_old; 

int line_sensor1;
int line_sensor2;
int ping = 9;

//only take front sensor readings if this counter is 0
int front_sensor_counter; 
//only take line sensor readings if this counter is 0
int line_sensor_counter1;
int line_sensor_counter2; 
...
void setup() {
	...
	line_sensor_counter1 = 0;
  line_sensor_counter2 = 0;
...
void read_sensors(){
  ...
  if(line_sensor_counter1 == 0 && line_sensor_counter2 == 0){ //starts reading line sensor when line_sensor_counter is 0
  
    line_sensor1 = analogRead(A2); //read the line sensor value
    line_sensor2 = analogRead(A3);
    
    if(line_sensor1 >= 700 && line_sensor2 >= 700){ //if the value is large, the white border is encountered, otherwise, the bot is still in the arena
      //set the counter so the back-off action will last approximately 1200 millisecond
      line_sensor_counter1 = 1200 ;  // reading period
      line_sensor_counter2 = 1200;
    ...
    line_sensor_counter1--;
    line_sensor_counter2--;
    return; //this return ensures the front sensor is NOT read while back-off action is in progress
  } 

  if(line_sensor1 >= 700 && line_sensor2 >= 0){ //if the value is large, the white border is encountered, otherwise, the bot is still in the arena
      //set the counter so the back-off action will last approximately 1200 millisecond
      line_sensor_counter1 = 1200 ;  // reading period
    
      //set speed and direction
      motor_turn_right(slower_speed);
      front_sensor_counter = 0; //reset front_sensor_counter because line sensor takes higher priority
       line_sensor_counter1--;
      return; //skip reading the front sensor because the white border has been detected (need to back-off immediately)
    } // if line_sensor is larger than 700, the program will simply move on to read the front sensor ()
   ...
   if(line_sensor1 >= 0 && line_sensor2 >= 700){ //if the value is large, the white border is encountered, otherwise, the bot is still in the arena
      //set the counter so the back-off action will last approximately 1200 millisecond
     // reading period
      line_sensor_counter2 = 1200;
      //set speed and direction
      motor_turn_left(slower_speed);
      front_sensor_counter = 0; //reset front_sensor_counter because line sensor takes higher priority
      return; //skip reading the front sensor because the white border has been detected (need to back-off immediately)
      line_sensor_counter2--;
    } // if line_sensor is larger than 700, the program will simply move on to read the front sensor ()
  ...
  if(front_sensor_counter == 0){ //starts reading front sensor when front_sensor_counter is 0
    ...
    front_sensor = analogRead(ping); //read the front sensor value
